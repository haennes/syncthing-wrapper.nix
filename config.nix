...
in
{
  config = lib.mkIf cfg.enable {
    ...
    let
      ...
      ensureDirExistsGen = attr: optional attr.ensure.DirExists "mkdir -p ${attr.path}";
      ensureOwner = attr: owner:
      let
        owner_opt = attr.ensure.owner;
      in
      optional owner_opt.owner "chown ${optionalString owner_opt.recursive "-R"} ${owner} ${attr.path}";
      ensureGroup = attr: group:
      let
        group_opt = attr.ensure.group;
      in
      optional group_opt.group "chown ${optionalString group_opt.recursive "-R"} :${group} ${attr.path}";
      ensurePermissions = attr:
      let
        permissions_opt = attr.ensure.permissions;
      in
      optional (permissions_opt.permissions != null) "chmod ${optionalString permissions_opt.recursive "-R"} ${permissions_opt.permissions} ${attr.path}";
    in
      lib.concatLines (
      ...
      ++ (flatten (mapAttrsToList(_: folder: mapAttrsToList(_: dir: ensureDirExistsGen dir) folder.path.${hostname}) folders))
      ...
      ++ (flatten(
        ...
        let
          ...
          mapAttrsToList(
            ...
            ++ lib.optionals (lib.hasAttrByPath [username "group"] config.users.users) (ensureGroup target_path config.users.users.${username}.group)
          ) target_paths
        ) folders
      ))
      ...
      ++ (flatten (mapAttrsToList(_: folder: mapAttrsToList(_: dir: ensurePermissions dir) folder.path.${hostname}) folders))
